...
def _parser_feuille(rows_list):
    ...
    for row in rows_list:
        ...
        tx = {
            "tiers": tiers, "nom_client": nom_map.get(tiers, tiers),
            "type_tx": type_tx, "reference": str(get(2, "")).strip(),
            "echeance": echeance, "jours": float(get(4, 0) or 0),
            "j60_plus": float(get(5, 0) or 0), "j30_59": float(get(6, 0) or 0),
            "j0_29": float(get(7, 0) or 0), "j_30_m1": float(get(8, 0) or 0),
            "j_60_m31": float(get(9, 0) or 0), "j_inf_60": float(get(10, 0) or 0),
            "categorie": ("facture" if type_tx in TYPES_FACTURES else "paiement" if type_tx in TYPES_PAIEMENTS else "avoir"),
        }
        tx["montant"] = tx["j60_plus"]+tx["j30_59"]+tx["j0_29"]+tx["j_30_m1"]+tx["j_60_m31"]+tx["j_inf_60"]
        transactions.append(tx)
    if not transactions:
        return pd.DataFrame(), pd.DataFrame(), meta
    df_tx = pd.DataFrame(transactions)
    df_clients = df_tx.groupby(["tiers","nom_client"]).agg(
        nb_transactions=("reference","count"), nb_factures=("categorie", lambda x:(x=="facture").sum()),
        nb_paiements=("categorie", lambda x:(x=="paiement").sum()), nb_avoirs=("categorie", lambda x:(x=="avoir").sum()),
        j60_plus=("j60_plus","sum"), j30_59=("j30_59","sum"), j0_29=("j0_29","sum"),
        j_30_m1=("j_30_m1","sum"), j_60_m31=("j_60_m31","sum"), j_inf_60=("j_inf_60","sum"),
        montant_total=("montant","sum"),
    ).reset_index()
    df_clients["total_overdue"] = df_clients["j60_plus"]+df_clients["j30_59"]+df_clients["j0_29"]
    df_clients["pct_overdue"] = (df_clients["total_overdue"]/df_clients["montant_total"].replace(0,pd.NA)*100).fillna(0).round(1)
    return df_tx, df_clients, meta

def charger_detailed_aged(fichier):
    nom = fichier.name.lower()
    contenu = fichier.read()
    sheets = _lire_xls_xlrd(contenu) if nom.endswith(".xls") and not nom.endswith(".xlsx") else _lire_xlsx_openpyxl(contenu)
    resultats = {}
...
def kpi_detailed(df_tx, df_clients):
    return {"nb_clients":len(df_clients),"nb_transactions":len(df_tx),
            ...
            "total_j60_plus":df_tx["j60_plus"].sum(),"total_j30_59":df_tx["j30_59"].sum(),
            "total_j0_29":df_tx["j0_29"].sum(),"total_non_echu":df_tx[["j_30_m1","j_60_m31","j_inf_60"]].sum().sum(),
            "total_overdue":df_tx[["j60_plus","j30_59","j0_29"]].sum().sum(),"balance_nette":df_tx["montant"].sum()}
...
def transactions_par_type(df_tx):
    return df_tx.groupby("type_tx").agg(nb=("reference","count"),montant=("montant","sum")).reset_index().sort_values("nb",ascending=False)

def flag_factures_impayees_60j(df_tx):
    mask = (df_tx["type_tx"]=="SAINV") & (df_tx["j60_plus"]>0)
    cols = [c for c in ["tiers","nom_client","reference","echeance","jours","j60_plus","montant"] if c in df_tx.columns]
...
def flag_solde_negatif(df_clients):
    ...
    cols = [c for c in ["tiers","nom_client","montant_total","nb_factures","nb_paiements","nb_avoirs"] if c in df_clients.columns]
...
def flag_echeances_depassees(df_tx, seuil_jours=90):
    ...
    cols = [c for c in ["tiers","nom_client","reference","echeance","jours","montant","j60_plus"] if c in df_tx.columns]
    return df_tx[mask][cols].sort_values("jours",ascending=False)

def resume_flags_detailed(df_tx, df_clients):
    return {"nb_factures_60j_plus":len(flag_factures_impayees_60j(df_tx)),
